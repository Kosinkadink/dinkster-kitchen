...
from dinkster_kitchen._rope_utils import (
    check_rope_inplace,
...
try:
    ...
    if os.path.exists(_module_path):
        _spec = importlib.util.spec_from_file_location(
            "dinkster_kitchen.backends.cuda._C", _module_path
        )
        if _spec and _spec.loader:
            _C = importlib.util.module_from_spec(_spec)
            sys.modules["dinkster_kitchen.backends.cuda._C"] = _C
...
from dinkster_kitchen.backends._activations import (  # noqa: E402
...
from dinkster_kitchen.backends._modulation import adaln_prep_modulation  # noqa: E402
from dinkster_kitchen.backends.eager import rope as _eager_rope  # noqa: E402
from dinkster_kitchen.backends.eager.group_norm_pad3d import (  # noqa: E402
    group_norm_silu_pad3d as _eager_group_norm_silu_pad3d,
)
from dinkster_kitchen.backends.eager.quantization import (  # noqa: E402
    DTYPE_CODE_TO_DTYPE,
    DTYPE_TO_CODE,
)
from dinkster_kitchen.backends.eager.quantization import (  # noqa: E402
...
from dinkster_kitchen.backends.eager.sol_attn import (  # noqa: E402
    _LOG2E,
    _block_lengths,
    _normalize_key_bias,
    _sink_count,
...
from dinkster_kitchen.backends.eager.svdquant import (  # noqa: E402
    _INT4_GROUP_SIZE,
    _unpack_int4_row_major,
)
from dinkster_kitchen.backends.eager.w4a8_int8 import (  # noqa: E402
...
from dinkster_kitchen.constraints import (  # noqa: E402
    DivisibleBy,
    ExactDims,
    FunctionConstraints,
    MinDims,
...
from dinkster_kitchen.float_utils import roundup  # noqa: E402
from dinkster_kitchen.registry import registry  # noqa: E402
from dinkster_kitchen.tensor.int8_utils import (  # noqa: E402
    _build_hadamard,
    _rotate_activation,
    _rotate_weight,
)
...
_FORCE_INT4_INT8_FALLBACK = os.environ.get("DINKSTER_KITCHEN_FORCE_INT4_INT8_FALLBACK", "0") == "1"
_INT4_PACKED_WEIGHT_SMALL_M_MAX = 8
_INT4_INT8_WEIGHT_CHUNK_N = max(1, int(os.environ.get("DINKSTER_KITCHEN_INT4_INT8_WEIGHT_CHUNK_N", "4096")))
_W4A8_CHUNKED = os.environ.get("DINKSTER_KITCHEN_W4A8_CHUNKED", "1") != "0"
_NVIDIA_16_SERIES = (
    "1660",
    "1650",
    "1630",
...
# Set DINKSTER_KITCHEN_DISABLE_CUTLASS=1 to force the cuBLAS int8 GEMM + separate
# dequant path (for benchmarking against the CUTLASS fused kernel).
_DISABLE_CUTLASS_INT8 = os.environ.get("DINKSTER_KITCHEN_DISABLE_CUTLASS", "0") == "1"
...
def quantize_svdquant_w4a4(
    ...
    lora_act_dtype = torch.float32 if os.getenv(
        "DINKSTER_KITCHEN_SVDQUANT_LORA_ACT_FP32", ""
    ).lower() in {"1", "true", "yes", "on"} else x.dtype
    stream_ptr = torch.cuda.current_stream(x.device).cuda_stream
    reuse_workspace = os.getenv(
        "DINKSTER_KITCHEN_SVDQUANT_REUSE_WORKSPACE", "1"
    ...
    # numerics. Set DINKSTER_KITCHEN_SVDQUANT_LORA_ACT_FP32=1 to force the older
...
def scaled_mm_svdquant_w4a4(
    ...
    tensor-core MMA. Set DINKSTER_KITCHEN_SVDQUANT_FUSE_LORA_UP=0 to force the
    ...
    Set DINKSTER_KITCHEN_SVDQUANT_FAST_ACCUM=1 to use the experimental packed
    ...
    fast_accum = os.getenv("DINKSTER_KITCHEN_SVDQUANT_FAST_ACCUM", "").lower() in {
        "1", "true", "yes", "on",
    }
    shared_scale_env = os.getenv("DINKSTER_KITCHEN_SVDQUANT_SHARED_SCALE")
    ...
    fuse_lora_env = os.getenv("DINKSTER_KITCHEN_SVDQUANT_FUSE_LORA_UP")
    fuse_lora = (
...
def gemv_awq_w4a16(
    ...
    Layouts (match dinkster_kitchen.backends.eager.awq):
