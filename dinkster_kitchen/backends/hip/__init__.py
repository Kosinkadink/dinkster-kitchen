...
from dinkster_kitchen._rope_utils import check_rope_inplace, trim_rope_freqs
from dinkster_kitchen.allocation import allocation_context
from dinkster_kitchen.backends import eager as _eager
from dinkster_kitchen.backends._activations import apply_input_act as _apply_input_act
from dinkster_kitchen.backends._activations import apply_residual as _apply_residual
from dinkster_kitchen.backends._activations import input_act_code as _input_act_code
from dinkster_kitchen.backends._activations import input_act_width as _input_act_width
from dinkster_kitchen.backends.eager import rope as _eager_rope
from dinkster_kitchen.backends.eager.quantization import DTYPE_CODE_TO_DTYPE, DTYPE_TO_CODE
from dinkster_kitchen.backends.eager.sol_attn import (
    _LOG2E,
    _block_lengths,
    _normalize_key_bias,
    _sink_count,
...
from dinkster_kitchen.backends.eager.w4a8_int8 import (
...
logger = logging.getLogger("dinkster_kitchen.hip")
...
try:
    ...
    if _module_path is None:
        ...
        _spec = importlib.util.spec_from_file_location("dinkster_kitchen.backends.hip._C", _module_path)
        _C = importlib.util.module_from_spec(_spec)
        sys.modules["dinkster_kitchen.backends.hip._C"] = _C
    ...
    if _C is not None and sys.modules.get("dinkster_kitchen.backends.hip._C") is _C:
        del sys.modules["dinkster_kitchen.backends.hip._C"]
...
def _tuning_env_int(name: str, default: int) -> int:
    ...
    typo raise would take out ``import dinkster_kitchen`` itself rather than just the
...
_W4A8_MAX_CHUNK_COLS = _tuning_env_int("DINKSTER_KITCHEN_W4A8_CHUNK_COLS", 4096)
...
_W4A8_CHUNK_MAX_ROWS = _tuning_env_int("DINKSTER_KITCHEN_W4A8_CHUNK_MAX_ROWS", 64)
...
def dequantize_convrot_w4a4_weight(
    ...
    # eager Hadamard.
    from dinkster_kitchen.backends.eager.convrot_w4a4 import _build_hadamard, _rotate_weight
...
def _adaln_impl(kernel, x, scale, shift, eps) -> torch.Tensor:
    ...
    from dinkster_kitchen.backends._modulation import adaln_prep_modulation
...
def _check_sol_args(sink_blocks, sink_q, topk_ratio, **tensors):
    ...
    """
    from dinkster_kitchen.constraints import sol_attn_common_call_rule
...
def _build_constraints(has_wmma: bool = True) -> dict:
    from dinkster_kitchen.constraints import (
        DivisibleBy,
        ExactDims,
        FunctionConstraints,
        MinDims,
...
# INT8 attention
#
# The public entry points live in dinkster_kitchen/sage_attention.py, which
...
#
# The public entry points live in dinkster_kitchen/gated_delta.py, which owns the
...
def flash_decode(
    ...
    launch so both backends can share dinkster_kitchen/flash_attention.py.
    """
...
def _register():
    ...
    # DINKSTER_KITCHEN_DISABLE_HIP=1 removes the backend from dispatch, leaving
    # triton/eager to handle every op.
    if os.getenv("DINKSTER_KITCHEN_DISABLE_HIP") == "1":
        registry.mark_unavailable("hip", "disabled by DINKSTER_KITCHEN_DISABLE_HIP=1")
        return
