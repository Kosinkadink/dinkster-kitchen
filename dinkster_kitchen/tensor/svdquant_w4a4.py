...
def _direct_cuda_backend(input_tensor: torch.Tensor, qdata: torch.Tensor):
    ...
    ComfyUI currently disables dinkster_kitchen's global CUDA backend on PyTorch
    ...
    """
    if not _env_enabled("DINKSTER_KITCHEN_SVDQUANT_DIRECT_CUDA", default=True):
        return None
...
class TensorCoreSVDQuantW4A4Layout(QuantizedLayout):
    ...
    # in dinkster_kitchen/backends/cuda/ops/{quantize,scaled_mm}_svdquant_w4a4.cu
